...
import { resolveAdminEmail, resolveAdminPersonName } from "./admin-account-target.mjs";
...
const personName = resolveAdminPersonName();
...
if (matchingUsers.length > 1) throw new Error("More than one Auth account exists for this address. No changes were made.");

let authUser = matchingUsers[0];
if (!authUser) {
  const created = await admin.auth.admin.createUser({ email, email_confirm: true, user_metadata: { name: personName } });
  if (created.error) throw created.error;
  authUser = created.data.user;
}

const person = await admin.from("people").select("id").eq("name", personName).maybeSingle();
if (person.error || !person.data) throw new Error(`${personName} person record was not found: ${person.error?.message ?? "missing"}`);
const contributor = await admin.from("contributors").select("id").eq("person_id", person.data.id).maybeSingle();
if (contributor.error || !contributor.data) throw new Error(`${personName} contributor record was not found: ${contributor.error?.message ?? "missing"}`);
...
if (memberships.length !== 1) throw new Error(`Expected exactly one app_members record for this person, found ${memberships.length}. No membership changes were made.`);
const membership = memberships[0];
if (membership.user_id && membership.user_id !== authUser.id) throw new Error("That membership is linked to a different Auth user. No changes were made.");
...
if (exactAuthMatches !== 1 || verification.data.user_id !== authUser.id || verification.data.person_id !== person.data.id || verification.data.contributor_id !== contributor.data.id || verification.data.role !== "admin" || !verification.data.active) throw new Error("Account verification failed.");
