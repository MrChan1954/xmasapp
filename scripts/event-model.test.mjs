...
test("migration 054 is the newest migration, and nothing else has been added", () => {
  ...
  // Roadmap Phase 3 adds 053: family invitation consent. It adds one nullable
  // column to `app_members`, four invitee routines and one delivery recorder,
  // and turns `claim_app_member()` into a `select false` stub. It creates no
  // table and changes no Event routine, so 025/026's promises are untouched.
  // Roadmap adds 054 on top: one redefined routine, so an invitation a family
  // administrator issued and its invitee explicitly accepted also approves the
  // Gift Planner account. No table, column, policy, grant or trigger.
  assert.equal(migrationFiles.at(-1), "202608100054_family_sponsored_approval.sql");
  assert.equal(migrationFiles.at(-2), "202608100053_family_invitation_consent.sql");
  assert.equal(migrationFiles.at(-3), "202608100052_global_account_approval.sql");
  assert.equal(migrationFiles.at(-4), "202608100051_drop_superseded_routines_and_narrow_table_grants.sql");
  assert.equal(migrationFiles.at(-5), "202608100050_audit_birthday_privacy_subject.sql");
  assert.equal(migrationFiles.at(-6), "202608100049_audit_area_from_acting_area.sql");
  assert.equal(migrationFiles.at(-7), "202608100048_revoke_area_helper_grants.sql");
  assert.equal(migrationFiles.at(-8), "202608100047_area_scoped_person_routines.sql");
  assert.equal(migrationFiles.at(-9), "202608100046_area_scoped_gift_idea_removal.sql");
  assert.equal(migrationFiles.at(-10), "202608100045_area_scoped_mutation_hardening.sql");
  assert.equal(migrationFiles.at(-11), "202608100044_area_scoped_person_administration.sql");
  assert.equal(migrationFiles.at(-12), "202608100043_birthday_planning_eligibility.sql");
  assert.equal(migrationFiles.at(-13), "202608100042_area_membership_lifecycle.sql");
  assert.equal(migrationFiles.at(-14), "202608100041_area_admin_handover.sql");
  assert.equal(migrationFiles.at(-15), "202608100040_own_birthday_wishlist.sql");
  assert.equal(migrationFiles.at(-16), "202608100039_area_aware_contributor_permissions.sql");
  assert.equal(migrationFiles.at(-17), "202608100038_acting_area.sql", "038 is still present, unedited");
