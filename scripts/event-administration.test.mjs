...
test("026 to 033 are still in order, with Phase 5's Areas on top", () => {
  ...
  // changes.
  assert.equal(files.at(-25), MIGRATION, "026 must come first of these");
  assert.equal(files.at(-24), REMINDER_MIGRATION, "then 027");
  assert.equal(files.at(-23), OCCASION_MIGRATION, "then 028");
  assert.equal(files.at(-22), BUDGET_MIGRATION, "then 029");
  assert.equal(files.at(-21), CONTRIBUTOR_MIGRATION, "then 030");
  assert.equal(files.at(-20), PRIVACY_MIGRATION, "then 031");
  assert.equal(files.at(-19), PEOPLE_MIGRATION, "then 032");
  assert.equal(files.at(-18), MEMBERSHIP_MIGRATION, "then 033");
  assert.deepEqual(files.slice(-17, -12), AREA_MIGRATIONS, "then Phase 5's five, in order");
  assert.deepEqual(files.slice(-12, -10), HARDENING_MIGRATIONS, "then the Q1 hardening");
  assert.deepEqual(files.slice(-10, -7), LIFECYCLE_MIGRATIONS, "and Q2's Area lifecycle on top of that");
  ...
  assert.equal(files.at(-7), "202608100044_area_scoped_person_administration.sql");
  assert.equal(files.at(-6), "202608100045_area_scoped_mutation_hardening.sql");
  // Q5 adds 046, which closes the one gift/purchase write 045 could not reach.
  assert.equal(files.at(-5), "202608100046_area_scoped_gift_idea_removal.sql");
  // Q6 adds 047: the four person routines that authorised in one family and
  // wrote in another.
  assert.equal(files.at(-4), "202608100047_area_scoped_person_routines.sql");
  ...
  assert.equal(files.at(-3), "202608100048_revoke_area_helper_grants.sql");
  ...
  assert.equal(files.at(-2), "202608100049_audit_area_from_acting_area.sql");
  ...
  assert.equal(files.at(-1), "202608100050_audit_birthday_privacy_subject.sql");
